...
import { Diseno } from '../entities/Diseno'; // <-- CAMBIO AQUÍ
import { Usuario } from '../entities/Usuario';
import { Prenda } from '../entities/Prenda';
import { Material } from '../entities/Material';

class DisenoController { // <-- CAMBIO AQUÍ
    public async create(req: Request, res: Response): Promise<Response> {
        ...
        const disenoRepository = AppDataSource.getRepository(Diseno); // <-- CAMBIO AQUÍ
        try {
            ...
            const nuevoDiseno = disenoRepository.create({ // <-- CAMBIO AQUÍ
                nombre,
                usuario,
                prenda,
                material,
    ...
    public async getAll(req: Request, res: Response): Promise<Response> {
        const disenoRepository = AppDataSource.getRepository(Diseno); // <-- CAMBIO AQUÍ
        try {
            const disenos = await disenoRepository.find({ // <-- CAMBIO AQUÍ
                relations: ['usuario', 'prenda', 'material'],
            });
            return res.status(200).json(disenos); // <-- CAMBIO AQUÍ
        } catch (error) {
...
export default new DisenoController(); // <-- CAMBIO AQUÍ